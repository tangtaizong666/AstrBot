...
class TestGetToolListPermission:
    def test_list_includes_permission_fields_for_non_builtin(self):
    ...
    def test_list_no_permission_fields_for_builtin(self):
        service = _make_tools_service()
        tools = service.get_tool_list()
...
class TestUpdateToolPermission:
    def test_set_admin_permission(self):
        service = _make_tools_service()
    ...
    def test_reject_builtin_tool(self):
        service = _make_tools_service()

        with pytest.raises(ToolsServiceError, match="Builtin tools"):
    ...
    def test_reject_invalid_permission_value(self):
        service = _make_tools_service()
        service.tool_mgr.func_list.append(_dummy_tool("target_tool"))

        with pytest.raises(ToolsServiceError, match="permission"):
            service.update_tool_permission(
